  ...
  const card = (
    ...
    >
      <ScrollReveal>
        ...
        <div className={`absolute top-4 right-4 w-14 h-14 rounded-full flex items-center justify-center transition-colors duration-300 ${isHovered ? "bg-[#00A896]" : "bg-[#0B1D3A]"}`}>
          <IconComponent size={24} className={isHovered ? "text-white" : "text-[#00A896]"} />
        </div>
        <div className="p-6">
          <h3 className="font-bold text-lg uppercase tracking-tight text-[#0B1D3A] mb-2 pr-16">{title}</h3>
          <p className="text-sm text-gray-600 line-clamp-3 mb-4">{shortDescription}</p>
          {previewMode ? (
            ...
            <Link href={`/servicios/${slug}`} className="inline-flex items-center text-[#00A896] text-sm font-medium hover:text-[#0B1D3A] transition-colors">
              Ver más
              <svg className="ml-1 w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" />
              </svg>
